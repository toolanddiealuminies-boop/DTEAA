...
console.log("- Tables in DTEAA not in muralineo:", diff1.length ? diff1.join(', ') : "None");

const diff2 = [...keys2].filter(x => !keys1.has(x));
console.log("- Tables in muralineo not in DTEAA:", diff2.length ? diff2.join(', ') : "None");
...
for (const t of commonTables) {
    ...
    if (missingIn2.length || missingIn1.length) {
        hasColDiff = true;
        console.log(`Table ${t}:`);
        if (missingIn2.length) console.log(`  - Missing in muralineo: ${missingIn2.join(', ')}`);
        if (missingIn1.length) console.log(`  - Missing in DTEAA: ${missingIn1.join(', ')}`);
    }
}
if (!hasColDiff) console.log("None");
...
for (const t of commonTables) {
    ...
    if (onlyIn1.length || onlyIn2.length) {
        hasFKDiff = true;
        console.log(`Table ${t}:`);
        if (onlyIn1.length) console.log(`  - FKs only in DTEAA: ${onlyIn1.join(', ')}`);
        if (onlyIn2.length) console.log(`  - FKs only in muralineo: ${onlyIn2.join(', ')}`);
    }
}
if (!hasFKDiff) console.log("None");
